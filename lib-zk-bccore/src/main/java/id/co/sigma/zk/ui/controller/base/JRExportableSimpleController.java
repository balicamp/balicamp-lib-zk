...
/**
 * Base simple controller with Jasper Report & Excel exportable feature.
 * @author <a href="mailto:ida.suartama@sigma.co.id">Goesde Rai</a>
 */
@org.springframework.stereotype.Component
public abstract class JRExportableSimpleController extends BaseSimpleController{

    private static final long serialVersionUID = -4911325292812872415L;
    ...
    public void doExport() {
        if (pdfRadio.isSelected()) {
            exportList(JRExportableSimpleController.EXPORT_TYPE_PDF);
        } else if (xlsRadio.isSelected()) {
            exportList(JRExportableSimpleController.EXPORT_TYPE_XLS);
        }
    }
    ...
    private void exportList(String documentType) {
        ...
        try {
            ...
            if (JRExportableSimpleController.EXPORT_TYPE_PDF.equals(documentType)) {
                Window reportWindow = (Window) Executions.createComponents("~./zul/pages/report/ReportView.zul", null, args);
                reportWindow.doModal();
            } else if (JRExportableSimpleController.EXPORT_TYPE_XLS.equals(documentType)) {
