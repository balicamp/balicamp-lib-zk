...
public abstract class BaseSimpleController extends SelectorComposer<Component> {
    ...
    /**
     * di pergunakan untuk memformat money. formatter sesuai dengan locale yang di
     * pakai, tanpa currency sign.<br/>
    ...
    protected void fillLOVControls() {
        ...
        if (indexedParamField.isEmpty())
            return;
        Map<String, CommonLOVHeader> vals = lovProviderService.getLOVAsMap(getCountryLocale(),
                indexedParamField.keySet());
        if (vals == null)
            return;
     ...
     * render lookup
     */
    protected void fillLooukup(Field field, CommonLOVHeader header, CustomDataFormatter<CommonLOV> dataRenderer)
            throws Exception {
    ...
    /**
     * ini untuk keperluan penambahan listener ke component tertentu override method
     * ini jika diperlukan
     */
    ...
    protected String createComboScript() {
        ...
        for (Field f : fields) {
            if (f.isAnnotationPresent(ListOfValue.class)) {
                ...
                try {
                    ...
                    if (comp instanceof Combobox) {
                        ...
                        comp.setWidgetListener("onSelect",
                                "try {" + "		if(event.data.reference.value != undefined) {"
                        ...
                        if (ann.dependency() == null || ann.dependency().length == 0) {
                            ...
                            for (LoVDependency dpd : ann.dependency()) {
                                if (dpd.isFilter()) {
                                    String val = null;
                                    try {
                                        val = ((Combobox) getSelf().getFellowIfAny(dpd.comboId())).getValue();
                                    } catch (Exception e) {
                                        ((Combobox) getSelf().getFellowIfAny(dpd.comboId())).clearErrorMessage();
                                    }
                        ...
                        if (ann.onDemand()) {
                            comp.setWidgetListener("onChanging", "onChangingCombobox('" + dbName + "',this,event);");
                        } else {
                        ...
                        comp.setWidgetListener("onCtrlKey", "listFocus(event, this);");
                        ...
                        listbox.setWidgetListener("onOK", "putSelectedValue(this);");

                        listbox.setWidgetListener("onClick", "putSelectedValue(this);");
                ...
                if (comp != null) {
                    ...
                    script.append("loadLOVCombo(").append("cWindow.$f('").append(comp.getId()).append("'),'")
        ...
        if (!"".equals(lastCombo)) {
            script.insert(0, "zAu.send(new zk.Event(cWindow,'onLoadCombodata',{comboId: '" + lastCombo
                    + "'},{toServer:true}));\n");
    ...
    @SuppressWarnings("unchecked")
    private final void loadComboData(final String func, final String dbName, final Event event,
            final ListOfValue annLOV) {
        String sFilter = "";
        ...
        Clients.response(new AuResponse(func, event.getTarget(), "{className: \"" + dbName + "\", id: \"" + id
                + "\", filter: \"" + sFilter + "\", isInit: " + isInit + ", " + " list: " + list.toString() + "}"));

    }

    private final List<ListOfValueItem> loadListOfValueItems(final String dbName, final String sFilter,
            final ListOfValue annLOV, String... dependencyFilter) {
        ...
        try {
            ...
            if (dbName.equalsIgnoreCase("Branch")) {
                ...
                /*
                 * lov = generalPurposeDao.list(dbName + " lov", "lov", (filters != null ?
                 * (SimpleQueryFilter[])filters.toArray(new SimpleQueryFilter[filters.size()]) :
                 * null), sortArgs);
                 */

                /*
                 * lov = branchDao.getBranchComboboxByUserLogin(getDefaultBranch().getId(),
                 * (filters != null ? (SimpleQueryFilter[])filters.toArray(new
                 * SimpleQueryFilter[filters.size()]) : null), sortArgs);
                 */
                lov = branchDao.getBranchComboboxByUserLogin(getDefaultBranch().getId(),
                        (filters != null ? (SimpleQueryFilter[]) filters.toArray(new SimpleQueryFilter[filters.size()])
                                : null),
                        sortArgs);
            ...
            if (lov != null) {
                ...
                for (Object o : lov) {
                    if (!("".equals(annLOV.codeField()))) {
                        String code = String
                                .valueOf(ExtendedBeanUtils.getInstance().getPropertyBlind(o, annLOV.codeField()));
                        String value = String
                                .valueOf(ExtendedBeanUtils.getInstance().getPropertyBlind(o, annLOV.valueField()));
                        String label = String
                                .valueOf(ExtendedBeanUtils.getInstance().getPropertyBlind(o, annLOV.labelField()));
        ...
        if (data instanceof Map) {
            ...
            if (!(headerid == null || "".equals(headerid))) {
                ...
                Clients.response(new AuResponse("loadComboboxData", event.getTarget(),
                        "{className: \"" + headerid + "\", id: \"" + id + "\", " + "filter: \"" + sFilter + "\", "
                                + "isLov: true, " + "list: " + list.toString() + "}"));
            }
        }
    }
    ...
    /**
     * helper unutk zul file. render tanggal sebagai date time( sampai dengan
     * second. format standard = format 24 jam)
     */
    ...
    protected final void showErrorMessage(String message) {
        Messagebox.show(message, Labels.getLabel("title.msgbox.error"),
                new Messagebox.Button[] { Messagebox.Button.OK }, new String[] { Labels.getLabel("action.button.ok") },
                Messagebox.ERROR, Messagebox.Button.OK, null);
    }
    
    protected final void showInformationMessage(String message) {
        Messagebox.show(message, Labels.getLabel("title.msgbox.information"),
                new Messagebox.Button[] { Messagebox.Button.OK }, new String[] { Labels.getLabel("action.button.ok") },
                Messagebox.INFORMATION, Messagebox.Button.OK, null);
    }

    protected final void showWarningMessage(String message) {
        Messagebox.show(message, Labels.getLabel("title.msgbox.attention"),
                new Messagebox.Button[] { Messagebox.Button.OK }, new String[] { Labels.getLabel("action.button.ok") },
    ...
    protected List<LookupDetail> getLookupDetails(String headerId) {
        ...
        SimpleQueryFilter[] filters = { new SimpleQueryFilter("headerId", SimpleQueryFilterOperator.equal, headerId) };
        SimpleSortArgument[] sorts = { new SimpleSortArgument("sequence", true) };
        try {
